...
var UserWhere = struct {
	...
	Hometown:  whereHelpernull_String{field: "\"users\".\"hometown\""},
}
...
var (
	userAllColumns            = []string{"id", "username", "password", "birth_date", "bio", "hometown"}
	userColumnsWithoutDefault = []string{"username", "password", "birth_date", "bio", "hometown"}
...
func (o *User) TargetUsers(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"followers\" on \"users\".\"id\" = \"followers\".\"target_id\""),
...
func (o *User) FollowerUsers(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		...
		qm.Where("\"followers\".\"target_id\"=?", o.ID),
	)
...
// LoadTargetUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadTargetUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.From("\"users\""),
		qm.InnerJoin("\"followers\" as \"a\" on \"users\".\"id\" = \"a\".\"target_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Username, &one.Password, &one.BirthDate, &one.Bio, &one.Hometown, &localJoinCol)
		if err != nil {
	...
	if singular {
		object.R.TargetUsers = resultSlice
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.TargetUsers = append(local.R.TargetUsers, foreign)
				if foreign.R == nil {
					foreign.R = &userR{}
				}
...
func (userL) LoadFollowerUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"users\".*, \"a\".\"target_id\""),
		qm.From("\"users\""),
		qm.InnerJoin("\"followers\" as \"a\" on \"users\".\"id\" = \"a\".\"follower_id\""),
		qm.WhereIn("\"a\".\"target_id\" in ?", args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		...
		for _, foreign := range resultSlice {
			...
			foreign.R.TargetUsers = append(foreign.R.TargetUsers, object)
		}
		return nil
	}
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				...
				foreign.R.TargetUsers = append(foreign.R.TargetUsers, local)
				break
			}
		}
	}
...
// AddTargetUsers adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.TargetUsers.
...
func (o *User) AddTargetUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		query := "insert into \"followers\" (\"follower_id\", \"target_id\") values ($1, $2)"
	...
	if o.R == nil {
		...
		o.R.TargetUsers = append(o.R.TargetUsers, related...)
	}
...
// SetTargetUsers removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.FollowerUsers's TargetUsers accordingly.
// Replaces o.R.TargetUsers with related.
// Sets related.R.FollowerUsers's TargetUsers accordingly.
func (o *User) SetTargetUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	removeTargetUsersFromFollowerUsersSlice(o, related)
	if o.R != nil {
		o.R.TargetUsers = nil
	}
	return o.AddTargetUsers(ctx, exec, insert, related...)
}

// RemoveTargetUsers relationships from objects passed in.
// Removes related items from R.TargetUsers (uses pointer comparison, removal does not keep order)
...
func (o *User) RemoveTargetUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"followers\" where \"follower_id\" = $1 and \"target_id\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.TargetUsers {
			if rel != ri {
				continue
			}

			ln := len(o.R.TargetUsers)
			if ln > 1 && i < ln-1 {
				o.R.TargetUsers[i] = o.R.TargetUsers[ln-1]
			}
			o.R.TargetUsers = o.R.TargetUsers[:ln-1]
			break
		}
	}
...
func removeTargetUsersFromFollowerUsersSlice(o *User, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// Sets related.R.TargetUsers appropriately.
func (o *User) AddFollowerUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		query := "insert into \"followers\" (\"target_id\", \"follower_id\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
	...
	for _, rel := range related {
		if rel.R == nil {
			rel.R = &userR{
				TargetUsers: UserSlice{o},
			}
		} else {
			rel.R.TargetUsers = append(rel.R.TargetUsers, o)
		}
	}
	return nil
}
...
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.TargetUsers's FollowerUsers accordingly.
// Replaces o.R.FollowerUsers with related.
// Sets related.R.TargetUsers's FollowerUsers accordingly.
func (o *User) SetFollowerUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "delete from \"followers\" where \"target_id\" = $1"
	values := []interface{}{o.ID}
	if boil.DebugMode {
		fmt.Fprintln(boil.DebugWriter, query)
		fmt.Fprintln(boil.DebugWriter, values)
	...
	removeFollowerUsersFromTargetUsersSlice(o, related)
	if o.R != nil {
...
func removeFollowerUsersFromTargetUsersSlice(o *User, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
		for i, ri := range rel.R.TargetUsers {
			if o.ID != ri.ID {
				continue
			}

			ln := len(rel.R.TargetUsers)
			if ln > 1 && i < ln-1 {
				rel.R.TargetUsers[i] = rel.R.TargetUsers[ln-1]
			}
			rel.R.TargetUsers = rel.R.TargetUsers[:ln-1]
			break
		}
	}
}
